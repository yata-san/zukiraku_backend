...
def run(session: Session):
    ...
    for item in score_data:
        ...
        # 選択肢の取得（question_id と label で）
        choice = session.query(Choice).filter_by(question_id=qid, label=label).first()
        if not choice:
            print(f"⚠️ 該当選択肢なし → Q{qid} - '{label}'")
            continue

        # すでに同じ choice_id と score_type が登録されているかチェック
        existing = session.query(ChoiceScore).filter_by(
            choice_id=choice.choice_id, score_type=score_type).first()
        if existing:
            print(f"スキップ：{choice.choice_id} - {score_type}")
            skipped += 1
        else:
            new_score = ChoiceScore(
